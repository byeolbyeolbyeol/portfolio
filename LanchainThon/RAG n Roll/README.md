...
📌 프로젝트 개요
많은 지원자들이 AI를 활용해 자소서를 작성하지만, 결과물은 예쁜 문장일 뿐 합격의 근거가 부족합니다.
...
JD 분석·구조화 - 필수 역량, 기술 스택, 암묵적 요구사항을 JSON으로 추출
구조 라벨링 - 자소서 문단을 [도입/문제/해결/성과/피고]로 자동 분류
정량 점수화 - JD 적합도 및 스토리텔링 품질을 객관적 지표로 산출
수정안 + 근거 - 문장 단위 수정 제안과 함께 합격 사례/JD 근거 제시
리포트 생성 - 클릭 시 수정본 자동 생성 및 상세 분석 리포트 다운로드
